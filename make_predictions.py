...
def make_predictions(data_set_name, num_folds, run_fold, recommender, metric, topn_score, topn_sample, num_batches,
    ...
    if recommender in ["random", "popularity", "implicit-mf", "user-knn", "item-knn"]:
        recommendations = recommend(algo=recommender_alg, users=user_batches[run_batch], n=500, n_jobs=1)
        ...
        for user in user_batches[run_batch]:
            user_recs = recommender_alg.recommend(user, matrix[user], N=500)
            recommendations = pd.concat([recommendations,
                                         pd.DataFrame({'item': user_recs[0], 'score': user_recs[1], 'user': user})])
    ...
    pd.DataFrame.to_csv(recommendations,
                        path_or_buf=f"{base_path_predictions}/{run_fold}_{num_folds}_{run_batch}_{PREDICTION_BATCH_FILE}")
